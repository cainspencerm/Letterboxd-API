...
public class ReviewComment {
    ...
    // The message portion of the comment in LBML. May contain the following HTML tags:
    // <br> <strong> <em> <b> <i> <a href=""> <blockquote>.
    private String commentLbml;

    // If Letterboxd moderators have removed the comment from the site, @removedByAdmin
    // will be true and @comment will not be included.
    private boolean removedByAdmin;

    // If the comment owner has removed the comment from the site, @deleted will be true
    ...
    // If the review owner has blocked the commenter, @blockedByOwner will be true and
    // @comment will not be included.
    private boolean blockedByOwner;

    // If the authenticated member posted this comment, and the comment is still editable,
    // this value shows the number of seconds remaining until the editing window closes.
    private int editableWindowExpiresIn;

    // The review on which the comment was posted.
    private ReviewIdentifier review;

    // The message portion of the comment formatted as HTML.
    private String comment;

    public ReviewComment(String json) {
        ...
        review = object.has("review") ? new ReviewIdentifier(object.get("review").toString()) : null;
    ...
    public ReviewIdentifier getReview() {
        return review;
    }
